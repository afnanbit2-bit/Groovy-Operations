...
window.dsMove=function(id,dir){
  ...
  const bal=d.balance||0,isIn=dir==='in';
  window._dsMoveCtx={bal,dir};
  _fabModal(`${isIn?'Add':'Use'} drawstring · ${_gpEsc(d.color)}`,`
    <div style="display:flex;align-items:center;gap:10px;background:#f7f7f8;border-radius:10px;padding:10px 13px;margin-bottom:14px">
...
window._dsPreview=function(){
  const el=document.getElementById('ds-preview'),ctx=window._dsMoveCtx;if(!el||!ctx)return;
  const q=parseFloat(document.getElementById('ds-qty')?.value)||0;
  const nb=ctx.bal+(ctx.dir==='in'?q:-q);
  const short=ctx.dir==='out'&&nb<0;
  el.textContent=`New balance: ${nb.toFixed(1)} m · ≈ ${Math.floor(Math.max(0,nb)/DS_M_PER_UNIT)} units${short?' — over stock!':''}`;
};
// Per-color movement log (in + out), with in/out/balance totals.
window.dsLogView=function(id){
  const d=allDrawstrings.find(x=>x._id===id);if(!d)return;
  const moves=allDrawstringMoves.filter(m=>(m.colorKey||_dsColorKey(m.color))===id).sort((a,b)=>(b.ts||0)-(a.ts||0));
  const tin=moves.filter(m=>m.type==='in').reduce((s,m)=>s+(m.qty||0),0);
  const tout=moves.filter(m=>m.type==='out').reduce((s,m)=>s+(m.qty||0),0);
  const rows=moves.length?moves.map(m=>{
    const isIn=m.type==='in',c=isIn?'#16a34a':'#dc2626';
    const noteClean=(m.note||'').replace(new RegExp('\\s*·?\\s*'+(m.units||0)+' units$'),'').trim();
    return`<div style="display:flex;justify-content:space-between;gap:10px;padding:9px 0;border-bottom:1px solid #f5f5f5">
  ...
  _fabModal(`Log · ${_gpEsc(d.color)}`,`
    <div style="display:flex;gap:8px;margin-bottom:12px">
      <div style="flex:1;background:#f0fdf4;border-radius:9px;padding:9px 11px"><div style="font-size:10px;color:#065f46;text-transform:uppercase;letter-spacing:.04em">Total in</div><div style="font-size:16px;font-weight:800;color:#16a34a">+${tin.toFixed(1)} m</div></div>
      <div style="flex:1;background:#fef2f2;border-radius:9px;padding:9px 11px"><div style="font-size:10px;color:#991b1b;text-transform:uppercase;letter-spacing:.04em">Total out</div><div style="font-size:16px;font-weight:800;color:#dc2626">−${tout.toFixed(1)} m</div></div>
      <div style="flex:1;background:#f7f7f8;border-radius:9px;padding:9px 11px"><div style="font-size:10px;color:var(--muted);text-transform:uppercase;letter-spacing:.04em">Balance</div><div style="font-size:16px;font-weight:800">${(d.balance||0).toFixed(1)} m</div></div>
    </div>
...
// Partial roll usage: a roll can be issued at less than its full weight when
// production only consumes part of it — the balance is minted back into stock as
// a remnant roll. A leftover below this cutoff (in the fabric's unit — kg, or m
// for meter-based suppliers) is treated as scrap: the whole roll is issued and
// no remnant is created, so tiny slivers don't clutter the stock list.
const FAB_REMNANT_MIN=1;
function _fabIssueSafeId(code){return String(code||'').replace(/[^a-z0-9]/gi,'_');}
// Clamp a roll's user-entered "used" weight to [0, full].
function _fabRollUse(r){const full=r.weight||0;const u=(r.useWeight==null?full:r.useWeight);return Math.max(0,Math.min(u,full));}
// Weight that actually LEAVES stock for this roll: the used amount when a real
// remnant will be minted, otherwise the whole roll (scrap rounded into usage).
function _fabRollIssuedWeight(r){const full=r.weight||0;const use=_fabRollUse(r);const leftover=parseFloat((full-use).toFixed(2));return leftover>=FAB_REMNANT_MIN?use:full;}
// Leftover that will be minted as a remnant (0 when below the scrap cutoff).
function _fabRollRemnant(r){const full=r.weight||0;const leftover=parseFloat((full-_fabRollUse(r)).toFixed(2));return leftover>=FAB_REMNANT_MIN?leftover:0;}
...
// One issue can span MULTIPLE fabrics (2-tone / colour-block garments). Group
// the picked rolls by fabric combo; the first group is the "primary" fabric.
function _fabIssueGroups(){
  const map=new Map();
  for(const r of _fabIssueRolls){
    const k=r.key||_fabIssueKey;if(!k)continue;
    if(!map.has(k))map.set(k,{key:k,fabType:r.fabType,gsm:r.gsm,color:r.color,unit:r.unit||'kg',rolls:[],weight:0});
    const g=map.get(k);g.rolls.push(r);g.weight+=_fabRollIssuedWeight(r);
  }
  return [...map.values()];
}
function _fabIssuePrimaryStock(){const g=_fabIssueGroups()[0];return g?(allFabricInventory.find(x=>x._id===g.key)||null):null;}
function _fabMainStock(){return _fabIssuePrimaryStock()||allFabricInventory.find(x=>x._id===_fabIssueKey)||null;}
function _fabMainWeight(){return _fabIssueRolls.reduce((a,r)=>a+_fabRollIssuedWeight(r),0);}
...
window.fabIssueRecalc=function(){
  ...
  const stock=_fabMainStock();
  const unit=stock?.unit||'kg';
  const used=_fabIssueRolls.reduce((s,r)=>s+_fabRollIssuedWeight(r),0);
...
function _fabIssueAdd(code){
  ...
  _fabIssueRolls.push({rollCode:rc,weight:roll.weight||0,useWeight:roll.weight||0,status:st,key:stock._id,fabType:stock.fabType,gsm:stock.gsm,color:stock.color,unit:stock.unit||'kg'});
...
// Per-roll "used" hint: shows what happens to the roll's balance — nothing
// (whole roll), a remnant kept in stock, or a sub-cutoff scrap counted as used.
...
function _fabIssueRenderSelected(){
  ...
  const groups=_fabIssueGroups();
  const total=_fabIssueRolls.reduce((s,r)=>s+_fabRollIssuedWeight(r),0);
...
window.submitFabricIssue=async function(){
  ...
  // An issue can span MULTIPLE fabrics (2-tone / colour-block). Group the
  // picked rolls by fabric combo; groups[0] is the primary fabric.
  const groups=_fabIssueGroups().map(g=>({...g,stock:allFabricInventory.find(s=>s._id===g.key)})).filter(g=>g.stock);
  if(!groups.length){showToast('Fabric stock not found.',true);return;}
  const primary=groups[0].stock;
  ...
  // Per-fabric split (weight + its own avg) recorded on the issue.
  const fabrics=groups.map(g=>({inventoryKey:g.key,fabType:g.stock.fabType,gsm:g.stock.gsm||0,color:g.stock.color,unit:g.stock.unit||'kg',rollCodes:g.rolls.map(r=>r.rollCode),rollsCount:g.rolls.length,weight:parseFloat((g.weight||0).toFixed(2)),avgConsumption:cutQty>0?parseFloat(((g.weight||0)/cutQty).toFixed(4)):0}));
  // Matching-color rib issued alongside (optional) — 4% of the TOTAL fabric weight.
  ...
  try{
    ...
    const article=fabrics.length>1?fabrics.map(f=>`${f.fabType} ${f.gsm||0}gsm ${f.color}`).join(' + '):`${primary.fabType} ${primary.gsm||0}gsm ${primary.color}`;
    ...
    for(let i=0;i<groups.length;i++){
      const g=groups[i];const codes=g.rolls.map(r=>r.rollCode);
      const opts={fabType:g.stock.fabType,gsm:g.stock.gsm,color:g.stock.color,unit:g.stock.unit||fabUnit,removeRollCodes:codes,partialUse,reservePO:po,note:`Issued to factory for ${po} by ${session.name}`,sourceCol:'gatepasses',sourceId:gpId};
      if(i===0){opts.extraWrites=[{ref:doc(db,'gatepasses',gpId),data:payload}];await _fabInvUpsert(opts);}
      else{try{await _fabInvUpsert(opts);}catch(e){showToast(`Issued, but decrement for ${g.stock.color} failed: ${e.message}`,true);}}
    }
