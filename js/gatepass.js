...
function renderGatePass(){
  ...
  <div class="gp-tabs">
    ...
    <button class="gp-tab" id="gptab-fabricin" onclick="window.switchGPTab('fabricin')">Fabric In</button>
  </div>
  ...
  return`<div class="card"><div class="card-title">Record fabric arrival</div>
    <div class="form-grid">
      ...
      <div class="field"><label>Auto fabric code</label>
        <input id="fab-code" readonly placeholder="Pick fabric type to generate" style="background:#eef2ff;color:#1e3a8a;font-weight:700;letter-spacing:.04em">
        <div style="font-size:10px;color:var(--muted);margin-top:3px">System-generated. Each roll will be tagged <span id="fab-code-roll-hint">CODE-R01</span>, <span id="fab-code-roll-hint-2">CODE-R02</span>…</div>
      </div>
...
window.refreshFabCode=function(){
  ...
  const prefix=_fabPrefix(fabType);
  const colorCode=color?_fabColorCode(color):'';
  const code=_nextFabCodeFor(prefix,colorCode);
  codeEl.value=code;
  const h1=document.getElementById('fab-code-roll-hint');if(h1)h1.textContent=code+'-R01';
  const h2=document.getElementById('fab-code-roll-hint-2');if(h2)h2.textContent=code+'-R02';
...
window.submitFabricIn=async function(){
  ...
  const unit=_fabUnitForSupplier(supplier);
  const fabCode=document.getElementById('fab-code')?.value||_nextFabCodeFor(_fabPrefix(fabType),_fabColorCode(color));
  const rolls=[];
  document.querySelectorAll('#fab-rolls-body .roll-row').forEach(row=>{
...
// ── Fabric type → prefix map (for auto fabric codes & roll IDs) ──
const FABRIC_PREFIX_MAP=[
  [/sublimation/i,'SUB'],
  [/jersey/i,'JRS'],
  [/terry/i,'TRY'],
  [/twill/i,'TWL'],
...
function _fabPrefix(fabType){
  if(!fabType)return'FAB';
  for(const[re,p]of FABRIC_PREFIX_MAP){if(re.test(fabType))return p;}
...
function _fabColorCode(color){
  ...
  for(const[re,p]of COLOR_CODE_MAP){if(re.test(c))return p;}
  return(c.replace(/[^a-z]/gi,'').slice(0,3).toUpperCase()||'XXX');
}
function _nextFabCodeFor(prefix,colorCode){
  const cc=colorCode||'';
  const re=cc
    ?new RegExp('^'+prefix+'-'+cc+'-(\\d+)$','i')
  ...
  return cc?`${prefix}-${cc}-${seq}`:`${prefix}-${seq}`;
}
...
function _gpRefreshActiveTab(){
  ...
  else if(gpActiveTab==='fabricin')renderFabricInList();
